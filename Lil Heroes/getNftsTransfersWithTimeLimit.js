...
async function getLastTransferTimestamp(collectionAddress, tokenId) { // We filter the last transfer of the given Nft Id, from any address, and then we retrieve the tx timestamp.
    return await api.log.getLogs(collectionAddress, 0, 14063806, "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef", "AND", undefined, "AND", undefined, "AND", web3.utils.padLeft(web3.utils.numberToHex(tokenId), 64))
    .then(json => {
        return(json["result"]);
...
async function getNftInfoByCollectionAndId(collectionAddress) {
    let nftLastTransferDate;
   
    for(let i = 0; i<ids.length; i++) {
        await getLastTransferTimestamp(collectionAddress, ids[i]).then(res => {
            nftLastTransferDate = res;
            nftLastTransferDate.shift();
        });
    
        nftLastTransferDate.map(elem => {
            let nftPastOwner = web3.utils.toHex(web3.utils.hexToNumberString(elem["topics"][2]));
    
            if(nftPastOwner in nftHolders) {
                nftHolders[nftPastOwner].push(ids[i]);
            } else {
                nftHolders[nftPastOwner] = [ids[i]];
            }
        })
...
getNftInfoByCollectionAndId("0xD78b76Fcc33cd416dA9d3D42f72649a23D7AC647");